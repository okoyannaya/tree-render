...
function checkBracketsBalance(input: string): boolean {
  let balance = 0;
...
function parseNestedString(input: string): NestedArray {
  input = input.trim();
...
function drawTree(
  ...
  arr.forEach((item, index) => {
    ...
    const baseDashes =
      dashes && !isArray(item) && itemStr.length === 1 ? dashes : BASE_PADDING;
    ...
    const nextSpaces = isMaxLength
      ? nextDashes.replace(/-/g, " ")?.slice(0, baseDashes.length - 1)
      : "";
...
document.getElementById("drawButton")!.addEventListener("click", () => {
  const input = (document.getElementById("treeInput") as HTMLTextAreaElement)
    .value;
